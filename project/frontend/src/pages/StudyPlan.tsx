...
import type { KnowledgeLevel, StudySession } from '@/types';
    ...
    return (
      <AppLayout>
        <div className="p-4 sm:p-6 lg:p-8">
          <PageHeader title="Your Personalized Study Plan" subtitle="Tell Cadence your goal and it'll build a real, AI-generated schedule." />
          <CreatePlanForm />
        </div>
      </AppLayout>
    );
...
function CreatePlanForm() {
  const { createGoal, saveAvailableTime, saveKnowledgeLevel, generatePlan, loadDemoData } = useAppData();
  ...
  const canSubmit = title.trim().length > 0 && subjects.trim().length > 0 && deadline.length > 0 && studyDays.length > 0;
  ...
  return (
    <Card padding="lg" className="max-w-2xl">
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="mb-2 block text-sm font-medium text-ink-700 dark:text-ink-200">Current knowledge level</label>
        ...
        <Button size="lg" fullWidth onClick={handleSubmit} loading={generating} disabled={!canSubmit}>
        ...
        <button
          ...
          className="w-full text-center text-xs font-medium text-ink-400 hover:text-primary-600 dark:hover:text-primary-400"
        >
