...
export function Progress() {
  const { progress, plan, achievements } = useAppData();
      ...
      <div className="p-4 sm:p-6 lg:p-8">
        ...
        <div className="mt-5">
          <Card hover>
            ...
            <div className="space-y-3">
              {achievements.map((a) => (
                <div key={a.id} className="flex items-center gap-3 rounded-xl border border-ink-200 px-4 py-3 dark:border-ink-800">
                  <div className={`flex h-9 w-9 items-center justify-center rounded-xl ${
                    a.unlocked ? 'bg-success-50 text-success-600 dark:bg-success-700/20' : 'bg-ink-100 text-ink-400 dark:bg-ink-800'
                  }`}>
                    <Award size={18} />
                  </div>
                  ...
                  <Badge tone={a.unlocked ? 'success' : 'neutral'}>
                    {a.unlocked ? 'Done' : a.progress !== undefined && a.goal !== undefined ? `${a.progress}/${a.goal}` : 'Locked'}
                  </Badge>
                </div>
              ))}
            </div>
