...
async function start() {
  ...
  if (!config.geminiApiKey) {
    console.warn('[nova-server] WARNING: GEMINI_API_KEY is not set. Document analysis will fail until it is configured.');
  }
  if (!config.youtubeApiKey) {
    console.warn('[nova-server] NOTE: YOUTUBE_API_KEY is not set. AI Tutor will not suggest videos until it is configured.');
  }
  ...
  await userStore.ensureDemoUser().catch((err) => {
    console.warn('[nova-server] Could not seed demo user:', err instanceof Error ? err.message : err);
  });
