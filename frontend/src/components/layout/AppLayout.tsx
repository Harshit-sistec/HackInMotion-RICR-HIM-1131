...
const NAV = [
  ...
  { to: '/app/subjects', label: 'Subjects & Mind Map', icon: BookOpen, end: false },
...
export function AppLayout({ children }: { children: ReactNode }) {
    ...
    <div className="flex h-full flex-col justify-between py-6">
      ...
      <div>
        ...
        <nav className="space-y-1.5 px-4">
          <span className={`block px-3 text-[9px] font-bold tracking-widest text-[var(--nova-text-muted)] uppercase mb-2 ${collapsed ? 'text-center' : ''}`}>
            {collapsed ? '—' : 'Main Menu'}
          </span>
          {NAV.map((item) => (
      ...
      <div className="space-y-4 px-4">
        ...
        {/* User Card */}
        <div className={`rounded-2xl border border-[var(--nova-border)] bg-slate-50 dark:border-[#1E293B] dark:bg-[#172033]/50 p-3 shadow-sm ${collapsed ? 'flex justify-center' : ''}`}>
          <div className="flex items-center gap-3">
            ...
            {!collapsed && (
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-[10px] text-[var(--nova-text-secondary)] dark:text-[#CBD5E1]">{user?.email}</p>
              </div>
            )}
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-[var(--nova-bg)] text-[var(--nova-text-secondary)] dark:bg-[#0B1220] dark:text-[#CBD5E1] transition-colors duration-300">
      ...
      <div className={`transition-all duration-300 ${collapsed ? 'lg:pl-[112px]' : 'lg:pl-[312px]'} pr-0 lg:pr-6 pt-4 pb-6`}>
        ...
        <header className="hidden lg:flex h-16 items-center justify-between border border-[var(--nova-border)] bg-white/92 rounded-3xl px-6 mb-6 shadow-[0_1px_3px_rgba(15,23,42,0.06)] dark:border-[#1E293B] dark:bg-[#111827]/92 backdrop-blur-md">
          <span className="text-[10px] font-bold tracking-widest text-[var(--nova-text-secondary)] dark:text-[#94A3B8] uppercase">Cadence Learning Hub</span>
