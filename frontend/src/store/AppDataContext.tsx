...
export function AppDataProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    async function loadData() {
      if (user) {
        const synced = await goalService.syncFromDatabase(user.id);
        if (synced.goal) setGoal(synced.goal);
        
        const planSynced = await studyPlanService.syncFromDatabase(user.id);
        if (planSynced) setPlan(planSynced);
      } else {
      ...
      try {
        const snap = await progressService.getSnapshot();
        setProgress(snap);
      } catch (e) {
        console.error(e);
      }
