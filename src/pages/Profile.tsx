        ...
        <div className="grid gap-5 lg:grid-cols-3">
          ...
          <Card padding="lg" className="lg:col-span-1">
            <div className="flex flex-col items-center text-center">
              ...
              {editing ? (
                <div className="mt-4 w-full max-w-xs">
                  ...
                  <Button size="sm" className="mt-3" fullWidth onClick={save}><Check size={14} /> Save</Button>
                </div>
              ) : (
                <>
            ...
            <div className="mt-6 space-y-3 border-t border-ink-200 pt-4 dark:border-ink-800">
              <InfoRow icon={Calendar} label="Joined" value={new Date(user?.createdAt ?? Date.now()).toLocaleDateString(undefined, { month: 'long', year: 'numeric' })} />
              ...
              {goal ? (
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {goal.topics.map((t) => (
                      <span key={t} className="rounded-lg bg-ink-100 px-3 py-1.5 text-xs font-medium text-ink-600 dark:bg-ink-800 dark:text-ink-300">{t}</span>
                    ))}
                  </div>
                </div>
              ) : (
            ...
            <Card>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3 rounded-xl border border-ink-200 px-4 py-3 dark:border-ink-800">
...
function StatBox({ icon: Icon, label, value, color }: { icon: typeof Flame; label: string; value: string; color: 'primary' | 'warning' | 'accent' | 'success' }) {
  const colors = {
