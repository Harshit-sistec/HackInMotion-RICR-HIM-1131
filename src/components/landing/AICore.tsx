    ...
    <div className="relative flex h-[350px] w-full items-center justify-center md:h-[450px] lg:h-[500px]">
      ...
      >
        ...
        <motion.div
          ...
          className="absolute inset-4 select-none pointer-events-none"
        >
            ...
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-xs font-bold ${
                  node.type === 'weak' ? 'text-coral-400' : node.type === 'strong' ? 'text-aurora-400' : 'text-primary-300'
                }`}
              >
                {node.val}
              </span>
...
function Stage3Timeline({ scrollYProgress }: { scrollYProgress: MotionValue<number> }) {
  ...
  const steps = [
    { time: '09:00', task: 'Dynamic Programming', duration: '45 min', range: [0.46, 0.50] },
    { time: '11:00', task: 'Graph Algorithms', duration: '60 min', range: [0.50, 0.54] },
          ...
          return (
            <div key={i} className="flex items-center gap-4 relative">
              ...
              >
                ...
                <Badge tone="neutral" className="text-[9px] px-1.5 py-0.5">{step.duration}</Badge>
              </motion.div>
            </div>
          );
        })}
    ...
    <div className="relative flex flex-col items-center justify-center w-[300px] h-[340px] pointer-events-auto">
      ...
      >
        ...
        <p className="text-[10.5px] leading-relaxed text-ink-200">
          "Your attention retention fell in Dynamic Programming. I've re-shuffled tomorrow's schedule to give you a 15-min diagnostic session."
        </p>
      </motion.div>
    </div>
  );
...
function Stage5Ecosystem({ scrollYProgress }: { scrollYProgress: MotionValue<number> }) {
  ...
  const nodes = [
    ...
    { title: 'Today\'s focus', val: 'DP Practice', icon: Target, color: 'text-primary-300 bg-primary-500/10', x: 125, y: -100 },
    { title: 'Retention Score', val: '81% Mastered', icon: TrendingUp, color: 'text-accent-300 bg-accent-500/10', x: -125, y: 100 },
    { title: 'Next concept', val: 'Graphs Node', icon: BookOpen, color: 'text-aurora-400 bg-aurora-500/10', x: 125, y: 100 },
  ];
